...
## Desarrollo de página `ShopCategory`
```sh
project07
|-- frontend
    |-- src 
            ...
            <div className="shopcategory-products">
                {allProducts.map((item, index) => {
                    if(props.category === item.category) {
                        return <Item ket={i} id={item.id} name={item.name} image={item.image} price={item.price}/>
                    }
                })}
            </div>
        </div>
...
## Desarrollo página `LoginSignUp.jsx`

```{.jsx filename="LoginSignUp.jsx"}
...
## Desarrollo página `Product.jsx`

```{.jsx filename="Product.jsx"}
...
const Product = () => {
    ...
    const product = allProducs.find((e) => e.id === Number(productID)); 
    return (
        <div>
...
## Componente `Breadcrum.jsx`

```{.jsx filename="Breadcrum.jsx"}
...
## Componente `ProductDisplay.jsx`

```{.jsx filename="ProductDisplay.jsx}
...
```{.jsx filename="ShopContextProvider.jsx"}
